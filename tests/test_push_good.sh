...
log "Testing /push"
assert_eq "{}" "$(curl -k -d '{"daily_active_users": 10, "timestamp": 20, "total_users": 123, "total_room_count": 17, "daily_messages": 9, "homeserver": "many.turtles"}' https://localhost:${port}/push 2>/dev/null)"
...
sleep 2
assert_eq "{}" "$(curl -k -d '{"daily_active_users": 456, "timestamp": 19, "homeserver": "many.turtles"}' https://localhost:${port}/push 2>/dev/null)"
assert_eq "10
